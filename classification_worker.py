...
import numpy as np
from st_dev_info import developer_info_simple_html
from automl.plot import confusion_metrix, roc, correlation_matrix_plotly
from automl.handle_null_value import contains_missing_value, remove_high_null, fill_null_values
from automl.preprocess import convert_to_numeric, remove_rows_with_empty_target, remove_duplicates
from automl.llm_service import decide_fill_null, decide_encode_type, decide_model, decide_target_attribute, decide_test_ratio, decide_balance
from automl.pca import decide_pca, perform_pca
from automl.util_automl import split_data, check_and_balance, fpr_and_tpr, auc, save_model, calculate_f1_score
from automl.classification_model import train_selected_model
from automl.util_general import select_Y, contain_null_attributes_info, separate_fill_null_list, check_all_columns_numeric, non_numeric_columns_and_head, separate_decode_list, get_data_overview, get_selected_models, get_model_name, count_unique, attribute_info, get_balance_info, get_balance_method_name

def detect_problem_type(data, target, verbose=0):
    cat_limit = 30  # The number of categories to name integers as classification
    float_limit = 15  # Limits the number of float variable categories for it to become cat var

    if isinstance(target, str):
        target = [target]  
    targ = str(target[0]) # Analyze the first (only one) target in to detect problem type
    
    # Problem detection
    if data[targ].dtype in [np.int64, np.int32, np.int16, np.int8]:
        if len(data[targ].unique()) <= 2:
            model_class = 'BinaryClassification'
        elif 2 < len(data[targ].unique().tolist()) <= cat_limit:
            model_class = 'MultiClassification'
        else:
            model_class = 'Regression'
    elif data[targ].dtype in ['float16', 'float32', 'float64']:
        if len(data[targ].unique()) <= 2:
            model_class = 'BinaryClassification'
        elif 2 < len(data[targ].unique().tolist()) <= float_limit:
            model_class = 'MultiClassification'
        else:
            model_class = 'Regression'
    elif data[targ].dtype == bool:
        model_class = 'BinaryClassification'
    else:
        if len(data[targ].unique().tolist()) <= 2:
            model_class = 'BinaryClassification'
        else:
            model_class = 'MultiClassification'
    return model_class

def decide_model_type(DF):
    attributes_for_target, types_info_for_target, head_info_for_target = attribute_info(DF)
    target_Y = decide_target_attribute(attributes_for_target, types_info_for_target, head_info_for_target)
    if target_Y != -1:
        model_class = detect_problem_type(DF, target_Y)
        if model_class == "BinaryClassification" or model_class == "MultiClassification":
            iModel = 1 # Classification Model
        else:
            iModel =2 # Regression Model
    else:
        st.info("AI cannot determine the problem type from the data. Please select the proper data analysis mode")
        iModel = 0
    return iModel
...
def classification_model_pipeline(DF):
    st.divider()
    ...
    if not st.session_state.target_selected:
        ...
        with st.spinner("AI is analyzing the data..."):
            ...
            st.session_state.target_Y = decide_target_attribute(attributes_for_target, types_info_for_target, head_info_for_target)
    ...
    if st.session_state.target_selected:
        ...
        if 'test_percentage' not in st.session_state:
            with st.spinner("Deciding testing set percentage based on data..."):
                st.session_state.test_percentage = int(decide_test_ratio(st.session_state.df_pca.shape) * 100)
            ...
            with st.container():
                ...
                if st.session_state.balance_data and "balance_method" not in st.session_state:
                    with st.spinner("AI is deciding the balance strategy for the data..."):
                        ...
                        st.session_state.balance_method = int(decide_balance(shape_info_balance, description_info_balance, balance_info_balance))
